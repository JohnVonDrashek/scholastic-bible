...
function parseDouayRheims() {
  ...
  // Pattern for book titles like "THE BOOK OF GENESIS" or "THE EPISTLE OF ST. PAUL TO PHILEMON"
  const bookTitlePattern = /^THE\s+(?:BOOK|EPISTLE|GOSPEL|ACTS|PROPHECY|LAMENTATIONS|CANTICLE)/i;
...
const BOOK_TITLE_MAP = {
  ...
  'THE FIRST BOOK OF KINGS': '1-samuel',
  'THE SECOND BOOK OF KINGS': '2-samuel',
  ...
  'THE CANTICLE OF CANTICLES': 'canticles',
  'THE BOOK OF WISDOM': 'wisdom',
  'THE BOOK OF ECCLESIASTICUS': 'ecclesiasticus',
  ...
  'THE HOLY GOSPEL OF JESUS CHRIST ACCORDING TO ST. MATTHEW': 'matthew',
  ...
  'THE EPISTLE OF ST. PAUL THE APOSTLE TO THE HEBREWS': 'hebrews',
  ...
  'THE CATHOLIC EPISTLE OF ST. JUDE THE APOSTLE': 'jude',
...
function generate() {
  ...
  for (const bookMeta of BOOK_ORDER) {
    ...
    for (const [title, intro] of bookIntros) {
      if (BOOK_TITLE_MAP[title] === bookMeta.id) {
        bookIntro = intro;
        break;
      }
    }
