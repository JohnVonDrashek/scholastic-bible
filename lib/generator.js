...
function parseDouayRheims() {
  ...
  let lastSavedVerseKey = null; // "book:chapter:verse"
  const verseCommentary = new Map(); // key: "book:chapter:verse" -> array of {lemma, text}
  ...
  // Pattern for Challoner commentary: "Word...." or "Word phrase...."
  const commentaryStartPattern = /^[A-Z][^.]*\.\.\.\./;
  ...
  /**
   * Parse accumulated commentary text into {lemma, text} annotations
   */
  function parseCommentaryAnnotations(rawText) {
    const annotations = [];
    // Split by the pattern "Word...." but keep the delimiter
    // Match: capital letter, any chars except period, then four dots
    ...
    for (const part of parts) {
      ...
      // Extract lemma (before "....") and text (after "....")
  ...
  function saveCommentary() {
    if (lastSavedVerseKey && commentaryText.length > 0) {
      const rawCommentary = commentaryText.join(' ').trim();
      const annotations = parseCommentaryAnnotations(rawCommentary);
      if (annotations.length > 0) {
        verseCommentary.set(lastSavedVerseKey, annotations);
      }
    }
    commentaryText = [];
    inCommentary = false;
  ...
  function saveCurrentVerse() {
    if (currentBook && currentChapter !== null && currentVerse !== null && currentText.length > 0) {
      ...
      // Track which verse we just saved for commentary association
      lastSavedVerseKey = `${currentBook}:${currentChapter}:${currentVerse}`;
    }
    currentText = [];
  }
  ...
  for (const line of lines) {
    ...
    if (inBookIntro && trimmed && !commentaryStartPattern.test(trimmed)) {
      bookIntro.push(trimmed);
      continue;
    }

    // Capture chapter summary (text between chapter header and first verse)
    if (inChapterSummary && trimmed && !commentaryStartPattern.test(trimmed)) {
      chapterSummary.push(trimmed);
      continue;
    }

    // Empty line ends the current verse, start looking for commentary
    ...
    // Empty line while in commentary mode - save accumulated commentary
    if (inCommentary && !trimmed) {
      saveCommentary();
      continue;
    }

    // Capture commentary text (lines after blank line following a verse)
    if (inCommentary && trimmed && lastSavedVerseKey) {
      commentaryText.push(trimmed);
      continue;
    }
  ...
  // Save last verse and any remaining commentary
  saveCurrentVerse();
  saveCommentary();

  return { books, chapterSummaries, bookIntros, verseCommentary };
}

/**
 * Parse supplementary content from the Douay-Rheims text
 * Includes: Glossary, Preface, Prayer of Manasses, 3rd/4th Esdras
 *
 * Section order in source file:
 * 1. Prayer of Manasses (140359)
 * 2. Third Book of Esdras (140398)
 * 3. Fourth Book of Esdras (141404)
 * 4. Preface to the Reader (143411)
 * 5. Hard Vvordes Explicated (143546)
 * 6. End of Gutenberg (143769)
 */
function parseSupplementary() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line === 'HARD VVORDES EXPLICATED') glossaryStart = i;
    if (line === 'THE PREFACE TO THE READER') prefaceStart = i;
    if (line.startsWith('THE PRAYER OF MANASSES KING OF IVDA')) prayerStart = i;
    if (line === 'THE THIRD BOOKE OF ESDRAS.') esdras3Start = i;
    if (line === 'THE FOVRTH BOOKE OF ESDRAS.') esdras4Start = i;
    if (line.includes('END OF THE PROJECT GUTENBERG')) gutenbergEnd = i;
  }

  // Parse Glossary
  const glossary = [];
  if (glossaryStart > 0 && gutenbergEnd > glossaryStart) {
    let currentTerm = null;
    let currentDef = [];

    for (let i = glossaryStart + 1; i < gutenbergEnd; i++) {
      const line = lines[i].trim();

      // Skip empty lines and section headers (single capital letters)
      if (!line || /^[A-Z]$/.test(line)) {
        // Save previous term if exists
        if (currentTerm && currentDef.length > 0) {
          const fullDef = currentDef.join(' ');
          // Extract verse references like [Matt 1:1] or [Rom 9:3]
          const refs = [];
          const refPattern = /\[([^\]]+)\]/g;
          let match;
          while ((match = refPattern.exec(fullDef)) !== null) {
            refs.push(match[1]);
          }
          // Remove refs from definition text
          const defText = fullDef.replace(/\s*\[[^\]]+\]/g, '').trim();
      ...
      // Check if this is a new term (starts with capitalized word followed by period)
      const termMatch = line.match(/^([A-Z][a-zA-Z\s,]+)\.\s*(.*)$/);
      if (termMatch && !currentTerm) {
        currentTerm = termMatch[1].trim();
        if (termMatch[2]) {
          currentDef.push(termMatch[2]);
        }
      } else if (currentTerm) {
  ...
  // Parse Preface (between prefaceStart and glossaryStart)
  // Paragraphs are separated by blank lines; lines within paragraphs should be joined with spaces
  let preface = '';
  if (prefaceStart > 0 && glossaryStart > prefaceStart) {
    const paragraphs = [];
    let currentPara = [];

    for (let i = prefaceStart + 1; i < glossaryStart; i++) {
      const line = lines[i].trim();
      if (line) {
    ...
    preface = paragraphs.join('\n\n');
  }

  // Parse Prayer of Manasses (between prayerStart and esdras3Start)
  let prayerOfManasses = '';
  if (prayerStart > 0 && esdras3Start > prayerStart) {
    const prayerLines = [];
    for (let i = prayerStart + 2; i < esdras3Start; i++) {
      const line = lines[i].trim();
      if (line) prayerLines.push(line);
    }
    prayerOfManasses = prayerLines.join(' ');
  }
  ...
  // Helper function to parse inline verses from text block
  // Format: chapter summary, then "AND text... (ref) 2 text... 3 text..."
  function parseInlineVerses(textBlock) {
    const verses = [];

    // Find where verse 1 starts - look for "AND " at start of sentence after summary
    // Summary typically ends with "...the Persians." and verse 1 starts with "AND "
    const verse1Idx = textBlock.search(/(?:^|Persians\.\s+)(AND\s)/i);
    if (verse1Idx < 0) {
      // Try alternate pattern - find first "AND " at sentence start
      const altIdx = textBlock.search(/\.\s+(AND\s[A-Z])/);
      if (altIdx >= 0) {
        const actualStart = textBlock.indexOf('AND ', altIdx);
        return parseVersesFromText(textBlock.substring(actualStart));
      }
      // Last resort - treat whole text as verse 1
      return [{ number: 1, text: textBlock.trim() }];
    }

    // Find the actual "AND " after the matched position
    const actualStart = textBlock.indexOf('AND ', verse1Idx);
    return parseVersesFromText(textBlock.substring(actualStart));
  }

  // Parse verses from text starting with verse 1
  function parseVersesFromText(text) {
    const verses = [];

    // Replace parenthetical scripture references with a placeholder to avoid matching numbers in them
    // e.g., "(4 Kings 23:21 / 2 Par 35:1)" -> "<<<REF>>>"
    const refs = [];
    ...
    // Use regex to find all verse boundaries in cleaned text
    // Pattern: look for ") N " or space + N + space followed by lowercase letter
    // The N must be at a word boundary, not inside a word
    const versePattern = /(?:>>>|[\s\)\.\:\;,])\s*(\d+)\s+(?=[a-z])/gi;
    ...
    const boundaries = [];
    while ((match = versePattern.exec(textWithPlaceholders)) !== null) {
      const num = parseInt(match[1], 10);
      // Only accept sequential or near-sequential verse numbers (allows gaps of up to 2)
      if (num > verseNum && num <= verseNum + 3) {
        boundaries.push({ pos: match.index, matchLen: match[0].length, num });
        verseNum = num;
      }
    }

    // Extract verses based on boundaries
    verseNum = 1;
    for (let i = 0; i < boundaries.length; i++) {
      const boundary = boundaries[i];
      // Text from lastEnd to this boundary is the current verse
      let verseText = textWithPlaceholders.substring(lastEnd, boundary.pos).trim();
      // Restore references
      verseText = verseText.replace(/<<<REF(\d+)>>>/g, (_, idx) => refs[parseInt(idx, 10)]);
      if (verseText) {
        verses.push({ number: verseNum, text: verseText });
      }
      verseNum = boundary.num;
      // Move past the matched pattern
      lastEnd = boundary.pos + boundary.matchLen;
    }

    // Don't forget the last verse
    let lastVerseText = textWithPlaceholders.substring(lastEnd).trim();
    lastVerseText = lastVerseText.replace(/<<<REF(\d+)>>>/g, (_, idx) => refs[parseInt(idx, 10)]);
    if (lastVerseText) {
      verses.push({ number: verseNum, text: lastVerseText });
    }

    // Final cleanup: ensure all placeholders are restored (including partial matches)
    for (let i = 0; i < verses.length; i++) {
      verses[i].text = verses[i].text
        .replace(/<<<REF(\d+)>>>/g, (_, idx) => refs[parseInt(idx, 10)] || '')
        .replace(/<<<REF(\d+)/g, (_, idx) => refs[parseInt(idx, 10)] || '')  // Handle unclosed
        .trim();
    }
  ...
  // Parse 3rd Esdras (between esdras3Start and esdras4Start)
  // Uses "CHAP. I." format for chapters, with inline verse numbers
  let esdras3 = { title: 'The Third Book of Esdras', chapters: [] };
  if (esdras3Start > 0 && esdras4Start > esdras3Start) {
    let currentChapter = null;
    let chapterText = [];

    for (let i = esdras3Start + 1; i < esdras4Start; i++) {
      const line = lines[i].trim();

      // Check for chapter header like "CHAP. I." or "CHAP. II."
      const chapterMatch = line.match(/^CHAP\.\s+([IVXLC]+)\.?$/);
      if (chapterMatch) {
        // Save previous chapter if any
        if (currentChapter !== null && chapterText.length > 0) {
          const fullText = chapterText.join(' ');
          const verses = parseInlineVerses(fullText);
          if (verses.length > 0) {
            esdras3.chapters.push({ number: currentChapter, verses });
          }
        }
  ...
  // Parse 4th Esdras (between esdras4Start and prefaceStart)
  // Uses same inline verse format as 3rd Esdras
  let esdras4 = { title: 'The Fourth Book of Esdras', chapters: [] };
  if (esdras4Start > 0 && prefaceStart > esdras4Start) {
    let currentChapter = null;
    let chapterText = [];

    for (let i = esdras4Start + 1; i < prefaceStart; i++) {
      const line = lines[i].trim();

      // Check for chapter header like "CHAP. I."
      const chapterMatch = line.match(/^CHAP\.\s+([IVXLC]+)\.?$/);
      if (chapterMatch) {
        // Save previous chapter if any
        if (currentChapter !== null && chapterText.length > 0) {
          ...
          const verses = parseInlineVerses(fullText);
          if (verses.length > 0) {
            esdras4.chapters.push({ number: currentChapter, verses });
          }
        }
        currentChapter = romanToNumber(chapterMatch[1]);
        chapterText = [];
...
function generate() {
  ...
  const { books: drBooks, chapterSummaries, bookIntros, verseCommentary } = parseDouayRheims();

  console.log('Parsing supplementary content...');
  const supplementary = parseSupplementary();
  ...
  console.log(`  Found ${verseCommentary.size} verses with Challoner commentary`);
  console.log(`  Found ${supplementary.glossary.length} glossary terms`);
  console.log(`  Found ${supplementary.esdras3.chapters.length} chapters in 3 Esdras`);
  console.log(`  Found ${supplementary.esdras4.chapters.length} chapters in 4 Esdras`);
  ...
  for (const bookMeta of BOOK_ORDER) {
    ...
    for (const chapterNum of sortedChapters) {
      ...
      for (const drVerse of drVerses.sort((a, b) => a.number - b.number)) {
        ...
        // Look up Challoner commentary using DR book name
        const commentaryKey = `${drBookName}:${chapterNum}:${drVerse.number}`;
        const challoner = verseCommentary.get(commentaryKey) || [];
        chapterChallonerCount += challoner.length;
      ...
      bookChallonerCount += chapterChallonerCount;
      ...
      chapters.push({
        ...
        challonerCount: chapterChallonerCount
      });
    }
    ...
    books.push({
      ...
      challonerCount: bookChallonerCount
    });
  }
  ...
  const output = { books, supplementary };
  ...
  console.log(`  ${supplementary.glossary.length} glossary terms`);
  console.log(`  Preface: ${supplementary.preface.length} characters`);
  console.log(`  Prayer of Manasses: ${supplementary.prayerOfManasses.length} characters`);
}
